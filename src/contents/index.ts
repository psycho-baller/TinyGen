...
const newVideoLoaded = async () => {
	...
	snipBtn = document.getElementsByClassName("snip-btn")[0] as
		| HTMLButtonElement
		| undefined;
	...
	if (!snipBtn) {
		...
		async function addSnipUsingKeyboardShortcut(e: KeyboardEvent) {
			...
			const isTyping =
				isTypingInTextArea || isTypingInInput || isTypingInComment;
...
async function addNewSnipEventHandler() {
	...
	const callingConcurrently = !addPlaceholderSnipToPreviewBar(
		startTime,
		currentTime,
	);
	...
	// if there exists a transcript, use it to get the summary, otherwise use the title
	if (vidTranscript.length > 0) {
		snipTranscript = await getSnipTranscript(
			vidTranscript,
			startTime,
			currentTime,
		);
		...
		const cleanedTitle = vidTitle.replace(/[\uD800-\uDFFF]./g, "");
		const encodedTitle = Buffer.from(cleanedTitle).toString("base64");
		const encodedSummary = vidSummary
			? Buffer.from(vidSummary).toString("base64")
			: "";
		summary = await fetch(`${URL}/llm/summarize/snip`, {
			method: "POST",
			headers: {
...
async function updateVideoSnips(snips?: Snip[]) {
	...
	// TODO: Optimize
	previewBar = document.getElementById(
		"snip-preview-bar",
	...
	snips.forEach((snip) => {
		...
		snipElement.style.backgroundColor =
			firstTag?.color || "hsl(161deg 55% 66%)";
...
// snip functions
function addPlaceholderSnipToPreviewBar(
