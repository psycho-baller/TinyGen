...
class GithubFileLoader:
    ...
    def get_file_paths(self, branch="main"):
        ...
        try:
            ...
            return [f for f in all_files if f["type"] == "blob"]
        except requests.exceptions.HTTPError as http_err:
            # Check if the error is because the branch doesn't exist or some other HTTP error
            if branch == "main":
                print(
                    f"Main branch not found or error occurred: {http_err}. Trying 'master' branch."
                )
    ...
    def is_file_in_excluded_files(self, file):
        ...
        return (
            any(part in self.excluded_dirs for part in path_parts)
            or path_parts[-1] in self.excluded_files
        )
